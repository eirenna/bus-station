...
(function (global, factory) {
  ( false ? undefined : _typeof(exports)) === 'object' && typeof module !== 'undefined' ? factory(__webpack_require__(/*! jquery */ "./node_modules/jquery/dist/jquery.js")) :  true ? !(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! jquery */ "./node_modules/jquery/dist/jquery.js")], __WEBPACK_AMD_DEFINE_FACTORY__ = (factory),
...
/* harmony import */ var _chenfengyuan_datepicker__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! @chenfengyuan/datepicker */ "./node_modules/@chenfengyuan/datepicker/dist/datepicker.js");
/* harmony import */ var _chenfengyuan_datepicker__WEBPACK_IMPORTED_MODULE_4___default = /*#__PURE__*/__webpack_require__.n(_chenfengyuan_datepicker__WEBPACK_IMPORTED_MODULE_4__);
...
jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).foundation(); //Change direction

var changeDirectionBtn = document.querySelector('.ba-direction-btn');

function changeDirection() {
  var from = document.querySelector('[name="from"]');
  var fromVal = from.value;
  var to = document.querySelector('[name="destination"]');
  var toVal = to.value;
  var btn = document.querySelector('.ba-direction-btn__img');
  btn.classList.toggle('ba-rotate');
  from.value = toVal;
  to.value = fromVal;
}

changeDirectionBtn.addEventListener('click', changeDirection); //Datepicker settings

var today = new Date();
jquery__WEBPACK_IMPORTED_MODULE_0___default()('[data-toggle="datepicker"]').datepicker({
  format: 'dd.mm.YYYY',
...
var blogSlider = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.ba-blog-slider');
blogSlider.slick({
  slide: '.ba-post',
  //autoplay: true,
  dots: true,
...
var departureCity = searchForm.querySelector('[name="from"]');
var arrivalCity = searchForm.querySelector('[name="destination"]');
var date = searchForm.querySelector('[name="date"]');
var seatsQty = searchForm.querySelector('[name="seats-quantity"]');
...
var routes = new XMLHttpRequest(); //Get data from address bar

var from = getUrlParameter('from');
var destination = getUrlParameter('destination');
var searchDate = getUrlParameter('date');
var seats = getUrlParameter('seats-quantity'); //When have all data, write it to the serach form's fields and make search
...
function searchFinalRoute(from, to, array) {
  ...
  if (intermediateRoute.length > 0) {
    for (var j = 0; j < intermediateRoute.length; j++) {
...
routes.onload = function () {
  ...
  var routesList = JSON.parse(routes.responseText); //Check if wanted departure and arrival points equal to race departure and arrival points

  var resultRoutes = findMatches(departureCity.value, arrivalCity.value, routesList);
  var resultCardTmpl = document.getElementById('result').innerHTML;
  var searchResults = document.querySelector('.ba-search-results');
  searchResults.innerHTML = '';
  resultRoutes.forEach(function (route) {
    var resultHTML = resultCardTmpl.replace(/⏰departure⏰/ig, route.departure).replace(/⏰arrival⏰/ig, route.arrival).replace(/⏰race-number⏰/ig, route.trip).replace(/⏰departuretime⏰/ig, route.departuretime).replace(/⏰departureStation⏰/ig, route.departureStation).replace(/⏰time⏰/ig, route.time).replace(/⏰arrivalStation⏰/ig, route.arrivalStation).replace(/⏰cost⏰/ig, route.cost).replace(/⏰arrivaltime⏰/ig, route.arrivaltime).replace(/⏰carrier⏰/ig, route.carrier).replace(/⏰bus⏰/ig, route.bus).replace(/⏰userDate⏰/ig, date.value).replace(/⏰wantedDeparture⏰/ig, departureCity.value).replace(/⏰wantedArrival⏰/ig, arrivalCity.value);
    searchResults.innerHTML += resultHTML;
  }); //Open and hide search result details

  var resultCard = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.ba-result-card');
  resultCard.find('.ba-more-button').on('click', function (e) {
    e.preventDefault();
    var chosenCard = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).closest(resultCard);
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).toggleClass('ba-more-button--opened');
  });
};

var searchFormAJAX = document.getElementById('search-form');

function showResults(e) {
  if (e) e.preventDefault();
  var userFrom = document.querySelector('.ba-user-choise__from');
  var userTo = document.querySelector('.ba-user-choise__to');
  var userDate = document.querySelector('.ba-user-choise__date');
...
module.exports = __webpack_require__(/*! d:\academy\academy\42-git-team\bus-station\src\assets\js\app.js */"./src/assets/js/app.js");
